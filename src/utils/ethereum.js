...
class Ethereum {
    ...
    async sendUniswapTransaction(trade, costOnly, wait, nonce){
        let deadline = trade.deadline
        let to = trade.to
        let path = trade.path
        let amountOutMin = trade.amountOutMin
        let exchangeAddress = trade.exchange.address;

        //Amount is 18 decimals
        //amount = _web3.utils.toWei(amount.toString(), "ether");

        const _uniswapExchange = new _web3.eth.Contract(_uniswapExchangeAbi, exchangeAddress);
        let tx = _uniswapExchange.methods.swapExactETHForTokens(amountOutMin, path, to, deadline);
        if(costOnly){
             return await this._getTransactionCost(50000); //TODO: Find the actual transaction cost?
        }
        else{
             let data = tx.encodeABI();
             return await this._broadcastTransaction(wallet, exchangeAddress, data, wait, nonce);
        }
    }
